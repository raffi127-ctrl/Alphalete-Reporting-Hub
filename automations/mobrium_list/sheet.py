...
THE TAB IS SORTED BY FIRST NAME, so an append would visibly break it. New people
are INSERTED at their alphabetical position instead. If the tab is ever found
out of order (someone appended by hand), we stop inserting and append instead —
re-sorting the whole tab would rewrite rows nobody asked us to touch.
[[don't touch user data without confirming]]

WRITES ARE ROW OPERATIONS, NOT A REWRITE. Everything we aren't changing is left
...
class Entry:
    ...
    def sort_key(self) -> str:
        """FIRST NAME ONLY — that is genuinely how the tab is ordered.

        The last name is not a tiebreak: 'Jennifer Nzekwe' sits above 'Jennifer
        Medina'. Including it made the whole tab read as unsorted on one pair
        and sent every new person to the bottom.
        """
        return self.first.strip().lower()
...
def insert_position(entries: List[Entry], first: str, last: str) -> int:
    """The 1-indexed sheet row a new person belongs on, keeping first-name
    order. Someone sharing a first name with people already there lands AFTER
    that group (strict `<`), which is where an eye would put them. Falls to just
    past the last entry when they sort last."""
