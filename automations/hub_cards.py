...
def _office_metrics_card() -> dict:
    ...
    return {
        ...
        "breakdown": (
            ...
            "knock, gap time, office totals. BROKEN UP BY TEAM (Raf, "
            "2026-09-13): the reps sit under their team's own totals band, "
            "read off the Team column of the office's sales board — an "
            "office without one keeps the flat board. Runs as its own "
            "scheduler entry "
...
AUTOMATED_REPORTS = [
    ...
    {
        "id": "below-the-mark",
        "name": "Recruiting - 1st showed up booked 2nd low numbers report",
        "category": "🎯 Recruiting",
        "creator": "Eve & Claude",
        "emoji": "🚩",
    ...
    # says "no run logged" forever — see day_orchestrator.test_hub_card_ids.
    {
        ...
        "breakdown": (
            ...
            "**•** **Reps who knock under one office but belong to "
            "another** come off that board and get their own. Carlos's "
            "fourteen have been dispositioning on Raf's Ownerville since "
            "2026-09-25, so Raf's board is his reps only and theirs is "
            "drawn from the same pull — same columns, same teal Chan Park "
            "line, their own total. Their board is TEXTED to Carlos's two "
            "iMessage rooms by the **Rep Gap Alerts** job on Lucy 1 (the "
            "box that can send iMessage, and the one already holding Raf's "
            "pull), so this run renders it and sends nothing.\n\n"
            ...
            "off its own evening.\n"
            "**•** **Never posts blank** — an office that genuinely logged no "
            "knocks is skipped and named in the log, never posted as an empty "
            "board. That is a quiet day, not a fault, and the card stays "
            "green.\n"
            "**•** **A broken pull turns this card RED.** An office whose "
            "scrape raises — a stalled Ownerville table, a dead endpoint — has "
            "no answer at all, so it counts as **failed**, not skipped, and "
            "the corrections channel hears about it. Until 2026-08-30 it "
            "counted as a skip, so a dead board exited clean and the card went "
            "green with an office silently missing its post.\n"
            "**•** One office failing, or one channel refusing an image, does "
            "not stop the others — the rest still post.\n\n"
            "WHO GETS IT\n"
            "**•** Everyone enrolled in **D2D metrics**, so enrolling an "
            "office enrols it here too.\n"
            "**•** **Isaiah is included** — wireless-only, so his board carries "
            "First/Last Knock, Gaps and Total Gaps but no knock counts or "
            "Talk-To columns (Ownerville has no Disposition page for a wireless "
            "office). He was held back until 2026-08-25, when gaps-only rows "
            "started rendering as a real board instead of 'no rows'.\n"
    ...
    },
    {
        ...
        "breakdown": (
            ...
            "The **Carlos** DM (Carlos, Elena, Valeria, Evelyn, Maud) was "
            "retired 9/4/26 at Carlos's request — his tab still fills, it just "
            "isn't DM'd to anyone.\n"
    ...
    },
    {
        ...
        "breakdown": (
            ...
            "'Both' → the rep is the 'User Name' column, the business is "
            "'Business Name', the customer is 'Customer Name' and the number "
            "is 'Phone' — all on the same row.\n\n"
            "Customer Type matters: it defaults to 'Residential', so a run "
            "that left it alone would come back with no B2B customers at all "
    ...
    },
    {
        ...
        "description": (
            ...
            "**lucyeco.streamlit.app/daily-dispositions**: they pick any number of "
        ...
        "breakdown": (
            ...
            "CARLOS\u2019S REPS GET THEIR OWN CARD, IN HIS OWN ROOMS\n"
            "Fourteen of his reps have been dispositioning on **Raf\u2019s** "
            "OwnerVille since 2026-09-25. They are off Raf\u2019s board and off "
            "his gap list \u2014 a leader in Raf\u2019s room cannot chase "
            "somebody else\u2019s rep \u2014 and their own board, with their own "
            "gap list and the same teal Chan Park line, is texted **hourly** to "
            "**NEW A Players** and **ATT B2B Leaders**. Same pull, no second "
            "login. The roster is `total_knocks.guests.GUEST_REPS`; the two rooms "
            "are `config.RAF[\"guests\"]`.\n\n"
        ...
        "actions": [
            {
                ...
                "icon": "\U0001F441",
                "primary": True,
                "help": "Pulls the Time Tracker, renders the card, and resolves every room it would post to — the Alphalete Partners chat, the Alphalete A-Team Chat and #alphalete-lvl1-chat — so you can see they are all still reachable. Sends nothing.",
    ...
    # ID SLUG-MATCHES the report id `gap_alerts_b2b`.
    {
        ...
        "description": (
            ...
            "Owners enroll themselves at "
            "**lucyeco.streamlit.app/daily-dispositions** and pick a B2B campaign; "
    ...
    },
    {
        ...
        "breakdown": (
            ...
            "**•** Names are corrected in **every** week it updates, the one "
            "in flight included — that is the week where a mismatch blocks "
            "an activation (changed 9/22; it used to skip the current week).\n"
