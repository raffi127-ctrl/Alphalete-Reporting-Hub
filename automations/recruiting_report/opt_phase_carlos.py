...
def write_icd_values(ws, icd_values: dict[int, object],
    ...
    if dry_run:
        ...
        # Go through fill._retry so a transient Sheets 429 (per-minute write
        # quota) waits out the 60s window and retries instead of crashing the
        # apply. This was the ONE write in the OPT apply path that bypassed the
        # shared retry every other Sheets call uses — hardening, not the fix for
