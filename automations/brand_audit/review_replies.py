...
def _positive_system(company_name: str) -> str:
    ...
    return (
        ...
        "  3. A brief closing line of appreciation.\n\n"
        "VARIETY IS CRITICAL. These publish in batches and sit next to each "
        "other on ONE public listing, so a shared skeleton is obvious to anyone "
        "scrolling. Vary the opening, the middle connector AND the closing.\n"
        "These are OVERUSED — use each only rarely, and NEVER two in one reply: "
        "\"We're glad …\", \"We appreciate your kind feedback\", \"means a great "
        "deal to our team\", \"means a lot to our team\", \"We are delighted to "
        "hear\", \"something we take great pride in\".\n"
        "Reach for genuinely different constructions: lead with what the "
        "reviewer described, or with the employee's name, or with the outcome "
        "they got — not always \"Thank you, {name}!\" followed by \"We're "
        "glad\". Close on the customer, the team, or the specific result. Vary "
        "the sentence count and rhythm too; not every reply is three tidy "
        "sentences.\n"
        "Keep the register consistent (warm, polished, 'we'), but the WORDS "
        "should look individually written by a person, not filled into a "
        "template.\n\n"
...
def _negative_system(company_name: str) -> str:
    ...
    return (
        ...
        "unprofessional conduct, or cult/MLM/scam/illegal claims): apply the "
        "rules below. Most such replies are SHORT — acknowledge the concern was "
        "raised (never conceding it), take it offline, sign off. The 'for "
        "context' paragraph is used only in the narrow case described below.\n"
        ...
        "THE 'FOR CONTEXT' PARAGRAPH IS RARE — use it SPARINGLY, not by default. "
        "ONLY when the review ATTACKS THE LEGITIMACY OR NATURE OF THE BUSINESS "
        "(calls it a cult, MLM, pyramid, scam or 'fake company', or claims the "
        "whole role/pay model is a lie) may you add ONE brief, plainly-worded "
        "sentence correcting that characterization — what the company actually "
        "is (a direct marketing company serving clients in the telecom and "
        "energy sectors; a model based on client services and performance, not "
        "recruitment). Correct it calmly and factually, never defensively, and "
        "VARY the wording — never a stock recited sentence.\n"
        "For EVERY OTHER complaint — a specific billing charge, one rep's "
        "conduct, a scheduling or pay dispute, a poor interview experience — DO "
        "NOT include the 'for context'/what-we-are paragraph, and DO NOT recite "
        "the transparency-role-compensation-training boilerplate at all. On a "
        "narrow complaint it reads canned and defensive. Just acknowledge the "
        "concern was raised (without conceding it), take it offline, sign off. "
        "The DEFAULT negative reply has NO context paragraph.\n\n"
...
def draft_reply(review: dict, company_name: str, feedback: str = "",
    ...
    if avoid and not negative:
        user += ("\n\nThese are the replies most recently published to this same "
                 "public listing. Yours will sit right next to them, so it must "
                 "NOT share their opening, connective phrasing or closing — a "
                 "reader scrolling the listing should not be able to tell they "
                 "came from a template. Study them and deliberately do "
                 "something different:\n"
