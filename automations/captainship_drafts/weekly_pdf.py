...
# Page geometry and the composer live in pdf_pages, shared with the daily
# attachments (daily_pdf) so the two documents print identically. Re-exported
# here because this module's callers and tests read them from weekly_pdf.
from automations.captainship_drafts.pdf_pages import (   # noqa: E402
    DPI, PAGE_MAX_PX, compose,
)
...
def _compose(pages: List[Tuple[str, Path]], out: Path) -> Path:
    """One board per page — see pdf_pages.compose."""
    return compose(pages, out)
