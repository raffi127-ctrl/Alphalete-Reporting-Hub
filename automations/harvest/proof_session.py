...
def _pairs_for_report(report_id: str) -> List[Tuple[str, str]]:
    ...
    if not needs and report_id == "fiber_activations":
        # Heavy import (pulls the browser stack), so only on demand.
        from automations.harvest.needs_fiber import fiber_needs
        needs = fiber_needs()
    if not needs and report_id == "captainship_activations":
        # Only the 13 shareable CaptainsBonus pulls — the 6 PSS pulls keep their
        # own logins regardless, so proving them would burn 18 extra pulls to
