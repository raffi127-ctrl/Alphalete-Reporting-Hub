...
AUTOMATED_REPORTS = [
    ...
    },
    {
        ...
        "breakdown": (
            ...
            "KNOWN GAP\n"
            "**Weeks 4/4/26 → 5/16/26 (7 weeks) have no source anywhere.** "
            "Adriana's last email from her personal Gmail was WE 3/28 (sent "
            "4/3); nothing reached this inbox again until 6/12, when Maud "
            "chased her and she resumed from her new **aptel.com** address. "
    ...
    },
    {
        ...
        "breakdown": (
            ...
            "MONDAYS — THE NEW WEEK'S BOARD\n"
            "The board holds one week at a time, set by the gold **WE** cell. "
            "Monday's **5:00am** run fills Sunday on the old board, which is "
            "correct. From **4:00pm** it needs the **new** week up — until it "
            "is, the fill **stops and waits** rather than writing on top of "
            "last week, and posts a message saying which week to set. Once the "
            "board is up the next hourly run fills it on its own.\n\n"
            "SAFETY GATES\n"
            "**•** **Only ever raises a number, never lowers one.** A rep is "
            "overwritten only when they post a **higher** count than the board "
            "already has — sales that reach the board another way stand.\n"
            "**•** Never writes into a week the board isn't showing — the same "
            "cell the 5:10am post checks, so the two can't disagree.\n"
...
# --------------------------------------------------------------------------
    ...
    if is_unassigned_view:
        ...
        st.markdown(
            ...
            # Captainship - Cancel Rate (ATT Fiber) — Eve's, in progress → purple
            # revisit item alongside the activation-rate card. (Megan 2026-07-28)
            "[class*='captainship-cancel-rate__calstat'] button{background:#EDE9FE!important;color:#5B21B6!important;border-color:#A78BFA!important;opacity:1!important;animation:none!important}"
            "</style>",
            unsafe_allow_html=True,
        )
