...
# --------------------------------------------------------------------------
# Report cards live in automations/hub_cards.py (split 2026-08-20 so that
# concurrent Claude sessions editing different reports stop colliding in
# this file). Card add/edit goes THERE; layout/nav here stays Megan-owned.
# --------------------------------------------------------------------------
from automations.hub_cards import (  # noqa: E402
    AUTOMATED_REPORTS,
    WEEKDAY_NAMES,
    _last_completed_we_sunday,
)
...
# (a pause only the Hub could see still got paged about two days later).
from automations.day_orchestrator.paused_reports import PAUSED_REPORTS  # noqa: E402
...
def _sched_time_lenient(s: str) -> dt.time | None:
    """Pull a clock time out of a schedule display string, which is often
    descriptive prose: '4 AM flow (when data's ready)' → 04:00, '9:30 AM CST'
    → 09:30, 'Mon 6pm' → 18:00. None if there's no time-like token at all."""
    s = (s or "").strip()
...
def _card_self_check() -> list[str]:
    """Boot-time lint for the miswired-card bug class (stuck orange pill,
    silent duplicate, unparseable schedule). Returns plain-English problem
    lines; empty list = all clear, render nothing. Cheap: reuses the run
    feed already loaded for the strips, plus one library-sheet read that
    fails soft (check skipped, never a false alarm)."""
    problems: list[str] = []
    ids = [r.get("id") or "" for r in AUTOMATED_REPORTS]

    for rid in sorted({i for i in ids if i and ids.count(i) > 1}):
        problems.append(
            f"two cards share the id `{rid}` — their pills and run history "
            "are indistinguishable; give one a new id")
    for r in AUTOMATED_REPORTS:
        if not r.get("id"):
            problems.append(f"card **{r.get('name', '(unnamed)')}** has no id "
                            "— its runs can never be matched to it")
        if not r.get("name"):
            problems.append(f"card `{r.get('id', '?')}` has no name")
        # Schedule times are often descriptive ('4 AM flow (when data's
        # ready)') — fine as long as SOME clock time is extractable. Only a
        ...
        if (_t and not r.get("hide_schedule")
            ...
            problems.append(
                f"card `{r.get('id', '?')}` has a schedule time with no "
                f"readable clock time in it ('{_t}') — the strips will "
                "treat it as 8:00 AM")
        _dr = r.get("daily_runs")
        if _dr is not None and not isinstance(_dr, (int, dict)):
            problems.append(
    ...
    # every known id (hardcoded + shared library). If the library read fails
    # or comes back empty, skip — a false "orphan" alarm is worse than none.
    try:
        from automations.day_orchestrator import hub_coverage as _cov
        ...
        if _lib_ids:
            ...
            for rid in orphans:
                ...
                if rid.endswith("-approved") and rid[:-9] in known:
                    continue
                problems.append(
                    f"runs are being logged under id `{rid}` but NO card has "
                    "that id — whatever publishes it has a typo'd report id, "
                    "and no pill will ever reflect those runs")
    except Exception:
        pass
    return problems
...
def _render_report_screenshot(report: dict) -> None:
    ...
    if shot.exists():
        ...
        st.markdown(
            # max-height (not a fixed height) so the frame hugs the image —
            # a wide/short screenshot no longer letterboxes with dead white
            # bands above/below; a tall one is still capped at 460px.
            "<div style='max-height:460px; border-radius:10px; "
...
if st.session_state.view == "home":
    ...
    # ---- Card wiring self-check (Megan 2026-08-20): boot-time lint for the
    # stuck-pill bug class. Invisible when everything is wired right.
    _probs = _card_self_check()
    if _probs:
        with st.expander(
                f"🧯 {len(_probs)} card wiring issue"
                f"{'s' if len(_probs) != 1 else ''} found — click for details",
                expanded=False):
            for _p in _probs:
                st.markdown(f"- {_p}")

    # ---- Needs attention (Megan 2026-08-20): morning triage at a glance —
    # ONLY reports that failed today, or are well past their scheduled time
    # with no run logged. Purely additive: Slack alerting stays the loud
    # path; this surfaces the same signal where Megan looks first. Renders
    ...
    for r in (AUTOMATED_REPORTS if _statuses else []):
        ...
        if _stt in ("failed", "partial"):
            ...
            continue
        # Never ran: only flag once it's clearly late — past its scheduled
        ...
        if _now > _late_after:
            _attention.append(
                (r, f"⏳ scheduled {_sched.get('time') or '8:00 AM'}, "
                    "no run logged"))
    if _attention:
        st.markdown("### 🚨 Needs attention")
        for r, _why in _attention:
            _who = (r.get("assignees") or ["Unassigned"])[0]
            _c1, _c2 = st.columns([5, 1])
            with _c1:
                st.markdown(
                    f"{r.get('emoji', '📋')} **{r['name']}** — {_why} · {_who}")
            with _c2:
                if st.button("Open", key=f"attn_{r['id']}",
                             use_container_width=True):
                    _go_user(_who)
                    st.rerun()
        st.markdown("---")
