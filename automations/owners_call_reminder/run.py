...
def send(dry_run: bool = True, final: "bool | None" = None) -> int:
    ...
    # Each sent reminder is one step of the "Promo Reminder Email" card — publish a
    # success so its pill climbs 1/3 → 3/3 green across the 11am/4pm/7:15pm sends.
    # (Split out of the Leader's Call card 2026-08-02.) Best-effort: a Hub write
    # must never fail the send.
    try:
        from automations.day_orchestrator import hub_publish
        hub_publish.publish_done("owners_call_reminder",
                                 "Promo Reminder Email", status="success")
    except Exception:
        pass
    return 0
