#!/bin/bash
#
# NEW LUCY — GUIDED setup. One command, and it asks you the rest.
#
# Generic version of lucy4_walkthrough.sh (2026-09-19). Proven end to end on
# Lucy 4; every fix it needed is folded in. The machine name is REQUIRED —
# a default here would provision a new box under an existing machine's name.
#
...
#
# Runs entirely AT THE NEW MACHINE. The credential pushes are queued onto Lucy 1's tab
...
NAME="${*:-}"
if [ -z "$NAME" ]; then
    echo "Usage: bash /tmp/lucy.sh Lucy 5     (the machine name is required)"
    exit 1
fi
...
This will ask you a handful of questions and do the rest itself.
  ...
  2. Four switches in System Settings       (it opens each one for you)
...
if yn "Open Messages now?"; then
    ...
    say "   Sign in as ${B}alphaletereporting@gmail.com${R} — the account every Lucy texts from."
    pause "Signed in? Press Return."
fi
say ""
note "If a test later shows red 'Not Delivered': Apple hasn't activated iMessage"
note "on this new Mac yet (the address box in Messages > Settings > iMessage is"
note "greyed out). Fix that cleared it on Lucy 4: Sign Out there, restart the"
note "Mac, sign back in. It is NOT the permission — that is step 3."

# -- remote login (SSH), so Megan's laptop can reach this box directly when the
# queue is busy. setup_lucy_machine.sh tries it, but macOS often refuses it from
# a script without Full Disk Access — Lucy 4 needed it by hand (2026-09-19).
say ""
if sudo -n systemsetup -getremotelogin 2>/dev/null | grep -q "On" \
   || nc -z -G 2 localhost 22 2>/dev/null; then
    ok "Remote Login (SSH) is already on."
else
    say "${B}d) Remote Login${R} — lets Megan's laptop reach this machine directly."
    if yn "Open Sharing now?"; then
        open "x-apple.systempreferences:com.apple.Sharing-Settings.extension" 2>/dev/null
        say "   Turn ON Remote Login. Leave Remote Application Scripting OFF —"
        say "   nothing we run uses it."
        pause "Done? Press Return."
    fi
fi
...
if [ -z "$NEEDED" ]; then
    ...
    if yn "Ask Lucy 1 to send them?"; then
        for k in $CRED_KEYS; do
            ...
            py -m automations.day_orchestrator.mini_control --by "$NAME setup" \
...
if [ "$A1" != "none" ] && [ -n "$A1" ]; then
    ...
    if [ "$A2" = "none" ] || [ -z "$A2" ]; then
        ...
        say "    and run nothing. Tell Megan's Claude: '$NAME session is not"
...
Last step is on MEGAN'S LAPTOP, not here — so her Claude can log in directly:

    ssh-copy-id $(whoami)@$(scutil --get LocalHostName 2>/dev/null).local

(it asks for THIS Mac's password once). Then tell Megan's Claude "$NAME is
done" — it verifies everything, and restarts this Mac once to prove it comes
back by itself.
TXT
hr
