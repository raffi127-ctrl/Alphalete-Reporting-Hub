...
case " $* " in
  *" --dry-run "*) : ;;
  *) "$VENV_PY" -c "from automations.day_orchestrator import hub_publish; hub_publish.publish_running('raf_captainship_bonus','Raf Captainship Bonus')" >> "$LOG_FILE" 2>&1 || true ;;
esac
